...
public class EmployeeService extends ServiceBase{
    ...
    /**
     * 指定されたページ数の一覧画面に表示するデータを取得、EmployeeViewのリストで返却
     * @param page ページ数
     * @return 表示するデータのリスト
     */
     ...
     * @return 取得データのインスタンス、取得できないならnull
     */
    public EmployeeView findOne(String code, String plainPass, String pepper) {
        ...
        try {
            ...
            //社員番号とハッシュ化済パスワードを条件に未削除従業員を一件取得する
    ...
    public long countBycode(String code) {
        ...
        //指定した社員番号の従業員の件数を取得
        long employees_count = (long)em.createNamedQuery(JpaConst.Q_EMP_COUNT_REGISTERED_BY_CODE_DEF, Long.class)
                                .setParameter(JpaConst.JPQL_PARM_CODE, code)
                                .getSingleResult();
